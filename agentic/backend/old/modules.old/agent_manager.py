...
try:
    from modules.config import AGENT_MANAGER_API_URL
except ImportError:
    AGENT_MANAGER_API_URL = os.environ.get("AGENT_MANAGER_API_URL", "localhost:3000")
...
def create_new_agent(agent_role):
    ...
    if not _validate_allowed_tools_for_agent_role(agent_role):
        raise ValueError(f"Allowed tools validation failed for agent type '{agent_role}'. "
