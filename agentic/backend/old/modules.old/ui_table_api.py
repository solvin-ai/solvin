...
"""
This module prints a summary table for API messages—that is, the messages that are going
to be sent on the wire to the OpenAI API. It prints the following columns:

      T # | M # | Role | Tool | Content

Only messages having built API payload (from UnifiedTurn.to_api_message()) are included.
...
A horizontal separator is inserted every 2 rows by default. However, turn 0 (with 3 messages)
is printed as a block (without an in‑group separator) and then forced to show a separator afterward.
All other turns (which have 2 messages each) are also forced to show a separator line after their rows.
"""
...
def print_api_table():
    ...
    every key in payload["messages"] is printed as a separate row. For the first row of a given
    UnifiedTurn, the Turn (T #) is shown; the Message ID (M #) is shown for every row.

    Normally a horizontal separator is inserted every 2 rows. This function has been modified
    so that:
      - For turn 0 (which has 3 messages) no separator is inserted mid‑turn;
        instead, a forced separator is added right after turn 0’s block.
      - For all other turns (with 2 messages each) after printing the rows using interval=2,
        a forced separator line is added to mark the group’s boundary.
    """
    console = get_console()
    messages = get_turns_list()

    table_title = "API Table (to be sent on the wire)"
    columns_config = [
    ...
    for msg in messages:
        ...
        rows_for_msg = []
        first_row = True       # Only the first row shows the turn.
        ...
        for key, part in payload["messages"].items():
            ...
            if key == "assistant":
                ...
                if payload.get("tool_meta", {}).get("normalized_args"):
                    args_str = json.dumps(payload["tool_meta"]["normalized_args"])
    ...
    # (Optional) sort the groups by turn number.
    ...
    # Apply sanitization and smart truncation (using the computed width) to every row.
    ...
    # For turn 0, we add its 3 messages with a separator_interval so high that no in‑group separator appears,
    # then force a section break.
    # For all other turns (with 2 messages each) we use separator_interval=2 (which would add an in‑group separator if applicable)
    # and then force a section break so the group boundary is clearly marked.
