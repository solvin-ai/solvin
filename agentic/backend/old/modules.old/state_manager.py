...
Description:
  ...
  - Updated config access to use direct indexing (e.g., config["SCRIPT_DIR"] and config.get("REPO_NAME", scope="service.repos")) instead of safe getters.
...
def get_turns_list_file(agent_role: str, agent_id: str) -> str:
    ...
    state_dir = _get_state_dir()
    repo_name = config.get("REPO_NAME", scope="service.repos")
    filename = f"{repo_name}__{agent_role}__{agent_id}_turns.json"
    return os.path.join(state_dir, filename)
...
def list_agent_states() -> list:
    ...
    for filename in os.listdir(state_dir):
        ...
        if filename.startswith(repo_name) and filename.endswith(".json") and "_turns" not in filename:
...
if __name__ == "__main__":
    ...
    config.get("REPO_NAME", scope="service.repos") = config.get("REPO_NAME", scope="service.repos")
    config["SCRIPT_DIR"] = config["SCRIPT_DIR"]

    test_agent_role = "root"
    test_agent_id = "001"
    logger.info("Loading state for agent type '%s' and id '%s'...", test_agent_role, test_agent_id)
    ...
    save_agent_state(state)
    print(f"Updated state for agent type '{test_agent_role}' and id '{test_agent_id}' saved.")
