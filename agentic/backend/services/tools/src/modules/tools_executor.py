...
def execute_tool(
    ...
    so that tools can read REPO_NAME, REPO_OWNER, REPO_DIR, JDK_VERSION, and METADATA from config.
    ...
    # 3) Inject initial context: repo name, passed owner, metadata
    ...
    logger.info(
        "Executing tool '%s' (turn_id=%s) in repo '%s/%s' (JDK=%s) with args:\n%s",
        tool_name, turn_id, info_owner, repo_name, jdk_version, pformat(args)
    )
    ...
    if isinstance(result, str):
        try:
            ...
            result_json = {"output_text": result}
        except Exception as e:
            err = f"Error parsing JSON response from tool '{tool_name}': {e}"
            logger.error(err)
            return {
                "execution_time": elapsed,
                "status": "failure",
    ...
    error_msg = "" if success else result_json.get("error", "")
