...
from shared.client_agents import clear_repo, health, ready, list_registry, delete_agent_role
...
def clear_repo_before_each_test():
    """
    Before each test, wipe out any running agents in TEST_REPO_URL.
    We ignore “nothing to clear” errors (422 or 404) here.
    """
    try:
        clear_repo(TEST_REPO_URL)
    except requests.HTTPError as e:
        code = getattr(e.response, "status_code", None)
        if code not in (404, 422):
            raise
    except Exception:
        # swallow any other error so tests can proceed
        pass
