...
def has_intervening_mutators(start_turn: int, current_turn: int, unified_registry: Dict[str, Any]) -> bool:
    """
    ...
    for turn in turns_list:
        ...
        if str(tool_obj.get("type", "")).lower() == "mutating":
            logger.debug("Intervening mutator in turn %d (tool=%s)", tnum, name)
            return True
    return False
...
def check_duplicate(
    ...
    if policy == "until-build":
        ...
        if last_turn and not has_intervening_mutators(last_turn.turn_meta["turn"], current_turn, unified_registry):
    ...
    for turn in reversed(turns_list):
        ...
        if not current_hash and normalized_key:
            if turn.tool_meta.get("normalized_filename", "") == normalized_key:
                candidate = turn
                break
    ...
    if ttype != "mutating":
        ...
        if has_intervening_mutators(cand_turn, current_turn, unified_registry):
