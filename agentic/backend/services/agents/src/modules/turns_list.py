...
"""
Turn list management: in‐memory LRU cache + SQLite persistence + monotonic turn IDs.

We use a TurnHistory class whose constructor loads and seeds turn-0 exactly once
(per conversation).  If the DB has no rows, we call initialize_initial_turn_history()
(which will error if history is non-empty).  Otherwise we simply cache the loaded turns.
"""
...
class TurnHistory:
    ...
    def __init__(self, agent_role: str, agent_id: str, repo_url: str):
        ...
        self.turns: List[UnifiedTurn] = _load_turns(repo_url, agent_role, agent_id) or []

        # 2) if no history at all, seed turn-0 (else do nothing)
        if not self.turns:
            # this will raise if history is non-empty
            initialize_initial_turn_history(self.turns, agent_role, agent_id, repo_url)
    ...
    @classmethod
    def get(cls, agent_role: str, agent_id: str, repo_url: str) -> "TurnHistory":
        """
    ...
    def save(self) -> None:
        """ Persist the in‐memory history to SQLite. """
        _save_turns(self.repo_url, self.agent_role, self.agent_id, self.turns)
    ...
    def add_turn(self, turn: UnifiedTurn) -> None:
        ...
        """
        last_idx, _ = load_state(self.repo_url, self.agent_role, self.agent_id)
        max_existing = max((ut.turn_meta.get("turn", -1) for ut in self.turns), default=-1)
    ...
    def delete_all(self) -> None:
        """
        Remove this agent/repo's entire turn history from memory and SQLite.
        """
        _delete_turns(self.repo_url, self.agent_role, self.agent_id)
...
# Free‐function wrappers for backwards compatibility:
...
def get_turns_list(
    ...
    """
    Return the in‐memory cached turns for this (agent_role, agent_id, repo_url).
    If no persisted turns exist, seeds turn-0 exactly once (else does nothing).
    """
...
def add_turn_to_list(
    ...
    """ Append a new turn (auto‐initializes turn‐0 only on first call). """
...
# alias for delete+persist
save_and_purge_turns_list = save_turns_list
