...
def _calculate_total_context_chars(turns_list: List[Any]) -> int:
    ...
    for turn in turns_list:
        ...
        if turn_number == 0:
            # For turn 0 (system/developer/user), count each message's content length.
...
def handle_context_violation(
    ...
    if latest_turn.tool_meta.get("rejection") == "context_exceeded":
        ...
        from modules.turns_list import save_and_purge_turns_list
        save_and_purge_turns_list(agent_role, agent_id, repo_url)
        return True

    valid, rejection_turn = _process_context_rejection(
        turns_list, current_turn, unified_registry
    )
    if not valid:
        logger.info(
            "Context limit exceeded at turn %d; applying rejection feedback.",
            current_turn
        )
        logger.debug("Rejection turn dump:\n%s", debug_dump_turn(rejection_turn))
        from modules.turns_list import save_and_purge_turns_list
        save_and_purge_turns_list(agent_role, agent_id, repo_url)
        return True
...
def _process_tool_call(
    ...
    if not allowed:
        ...
        if ut.tool_meta.get("rejection"):
            ...
            ut.messages["tool"]["raw"].update({
                ...
                "content":      "Rejection: " + ut.tool_meta["rejection"],
            })
    ...
    if unified_turn.tool_meta.get("rejection"):
        ...
        unified_turn.messages["tool"]["raw"].update({
            ...
            "name":         tool_name,
            "tool_call_id": call_id,
            "content":      "Rejection: " + unified_turn.tool_meta["rejection"],
        })
        logger.info(
    ...
    if result.get("status") != "success":
        ...
        unified_turn.messages["tool"]["raw"]["content"] = f"Failure: {err}"
...
def handle_assistant_turn(
    ...
    for idx, (tool_env, tool_meta) in enumerate(zip(tools_env, tools_meta)):
        ...
        # Immediately process this tool call (exec or reject)
        _process_tool_call(turn_num, unified_registry, repo_owner, repo_name)
