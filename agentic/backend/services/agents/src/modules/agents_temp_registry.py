...
def list_registry() -> List[AgentRegistryItem]:
    ...
    data = resp.json()
    # The remote API may wrap the list under 'agentTypes'
...
def delete_agent_role(agent_role: str) -> dict:
    ...
    # Try to fetch first; if it's already gone, return success
...
def get_agent_role(agent_role: str) -> Optional[AgentRegistryItem]:
    ...
    resp.raise_for_status()
    data = resp.json()
    # The remote API may wrap the single agent data in different ways
