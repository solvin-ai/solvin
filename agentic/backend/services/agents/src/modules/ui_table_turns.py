...
def print_turns_table():
    ...
    for turn in sorted(turn_groups.keys()):
        ...
        # Skip turns that never actually invoked a tool
        # (no "tool" payload or empty tool_name).
    ...
    total_context = sum(
        ...
        for msg in messages
        if extract_payload_meta(msg)[1].get("turn") != 0
    ) / 1024.0
...
def write_report(report_contents):
    ...
    """
    _, _, repo_url = get_current_agent()
...
if __name__ == "__main__":
    ...
    # Set up the current agent context for testing.
    _, _, repo_url = get_current_agent()
    print_turns_table()
