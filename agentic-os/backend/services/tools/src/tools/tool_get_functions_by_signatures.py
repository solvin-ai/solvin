...
def tool_get_functions_by_signatures(requests: list) -> dict:
    ...
    # we will flip this to True if *any* resolution or extraction fails
    error_flag = False
    ...
    for orig_fp, sig_list in grouped_requests.items():
        try:
            ...
            check_path(safe_fp, allowed_root=repo_root)
        ...
        if not os.path.isfile(safe_fp):
            ...
            final_result[safe_fp] = {
                s: f"File not found: {mask_output(safe_fp)}"
                for s in sig_list
            }
            continue
        ...
        try:
            ...
            error_flag = True
            final_result[safe_fp] = {
                s: f"Error reading file: {e}" for s in sig_list
            }
            continue
    ...
    # top-level success false whenever *any* lookup/extraction failed
    success = not error_flag
    return {
        "success": success,
        "error": "" if success else "One or more signatures could not be extracted",
        "output": final_result
    }
