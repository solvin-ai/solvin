...
Feature flags in shared.config:
  INVOCATION_REASON_ENABLED (bool, default False)
    Injects a required "invocation_reason" string property into every tool schema.
  TURNS_TO_PURGE_ENABLED (bool, default False)
    Injects an optional "turns_to_purge" integer property into every tool schema.
"""
...
# Schema snippets for optional injection
COMMON_INVOCATION_REASON = {
    "invocation_reason": {
        "type": "string",
        "description": "A one-sentence summary of why the assistant is calling this tool"
    }
}
COMMON_TURNS_TO_PURGE = {
    "turns_to_purge": {
        "type": "integer",
        "description": "Optional: list of turn (pair of assistant/tool messages) sequential numbers that may be purged from chat memory"
    }
}
...
def _make_tool_executor(tool):
    ...
    # Base JSON schema from the tool spec (may be None)
    json_schema = None
    if "function" in tool and "parameters" in tool["function"]:
        json_schema = tool["function"]["parameters"]

        # Inject common fields if enabled via config
        if config.get("INVOCATION_REASON_ENABLED", False) or config.get("TURNS_TO_PURGE_ENABLED", False):
            schema = deepcopy(json_schema)
            props = schema.setdefault("properties", {})

            if config.get("INVOCATION_REASON_ENABLED", False):
                props.update(COMMON_INVOCATION_REASON)
                req = schema.setdefault("required", [])
                if "invocation_reason" not in req:
                    req.append("invocation_reason")

            if config.get("TURNS_TO_PURGE_ENABLED", False):
                props.update(COMMON_TURNS_TO_PURGE)
