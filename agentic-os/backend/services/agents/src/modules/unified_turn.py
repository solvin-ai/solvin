...
class UnifiedTurn:
    ...
    def create_turn(
        ...
        # ----------------------------------------------------------------------------
        # Preserve any extra keys (like invocation_reason, turns_to_purge) in turn_meta
        turn_meta: Dict[str, Any] = {}
        # Copy everything except tool_meta/total_char_count (we'll override those)
        for k, v in meta.items():
            if k not in ("tool_meta", "total_char_count"):
                turn_meta[k] = v

        # Now enforce our canonical fields
        turn_meta["turn"]             = turn_number
        turn_meta["finalized"]        = bool(meta.get("finalized", True))
        turn_meta["total_char_count"] = tcc
        # ----------------------------------------------------------------------------
