...
Now scoped to (agent_role, agent_id, repo_url).
"""
...
def get_messages_list(
    ...
    """
    Read‐only: return every message (across all turns) for this agent/task,
    optionally filtered by role or by a single turn_id.
    """
    turns: List[UnifiedTurn] = get_turns_list(
...
def append_messages(
    ...
    **extra_fields: Any
    ...
    # 2) allocate one message_id per entry
    msg_ids = [
        allocate_next_message_id(repo_url, agent_role, agent_id)
        for _ in msgs
    ]

    # 3) build the new turn shell
    ...
    ut = UnifiedTurn.create_turn(
        ...
        {}   # messages will be injected below
    )

    # 4) inject each message
