...
def patch_requests(monkeypatch):
    ...
    Monkeypatch client.requests.get so that any GET to
      /turns/list, /turns/get, or /turns/metadata
    ...
    def fake_get(url, params=None, headers=None):
        ...
        # handle /turns/metadata
        if url.endswith("/turns/metadata"):
            envelope = {
                "data":   DUMMY_METADATA,
                "meta":   None,
                "errors": []
...
def test_get_metadata():
    """
    Verify that GET /turns/metadata returns our dummy metadata payload.
    """
    md = client.get_metadata("foo", "run123", "myrepo")
    assert isinstance(md, dict)
    assert md == DUMMY_METADATA
